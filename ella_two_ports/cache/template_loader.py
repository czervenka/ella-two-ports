...
from ella_two_ports.cache.utils import cache_this
...
def get_key(func, template_name, template_dirs=None):
    return 'ella_two_ports.cache.template_loader:%d:%s' % (settings.SITE_ID, template_name,)
...
def get_sel_template_key(func, template_list):
    return 'ella_two_ports.cache.template_loader.select_template:%d:%s' % (settings.SITE_ID, ','.join(template_list))
